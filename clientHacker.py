...
class Client:
    ...
    def __init__(self, port=45000, msges=25):
        ...
        try:
            ...
            try:
                # Dont send first hand shake
                self.msgout("START", "Dont send first hand shake")
                message = 'com-0 accept'
                self.send(message)
                rdata = self.receiveWait()
                # force connection close
                self.send("close")
                self.msgout("END", "Dont send first hand shake")
            except Exception as tmsg:
                self.msgout(str(tmsg), "Dont send first hand shake")
                self.msgout("END", "Dont send first hand shake")
            ...
            try:
                # Use fake IP Address
                self.msgout("START", "Use fake IP Address")
                message = 'com-0 99.0.0.1'
                self.send(message)
                rdata = self.receiveWait()
            except Exception as tmsg:
                self.msgout(str(tmsg), "Use fake IP Address")
                self.msgout("END", "Use fake IP Address")
            ...
            try:
                # Forget to send last accept
                self.msgout("START", "Forget to send last accept")
                ...
                # force connection close
                self.send("close")
                self.msgout("END", "Forget to send last accept")
            except Exception as tmsg:
                self.msgout(str(tmsg), "Forget to send last accept")
                self.msgout("END", "Forget to send last accept")
            ...
            try:
                # Send message number out of order
                self.constatus = "connecting"
                self.msgout("START", "Send message number out of order")
                message = 'com-0 ' + self.address
                self.send(message)
                rdata = self.receiveWait()
                if self.constatus == "connecting":
                    if rdata == ("com-0 accept " + self.address):
                        ...
                        self.send("msg-1000=Ok, good to know")
                        rdata = self.receiveWait()
                # force connection close
                self.send("close")
                self.msgout("End", "Send message number out of order")
            except Exception as tmsg:
                self.msgout(str(tmsg), "Send message number out of order")
                self.msgout("END", "Send message number out of order")
    ...
    def receiveWait(self):
        rdata = self.receive()
        i = 0
        while rdata is None:
            rdata = self.receive()
            if i > 5:
                raise Exception("\t\treceiveWait: Timeout")
            else:
                i = i + 1
        return rdata
